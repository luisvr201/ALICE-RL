function patch_jm(_jm) {
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      const objectStore = transaction.objectStore("dataStores");
      const request = objectStore.put({ id: 2, value: data });
  ...
  //       for (let j = i - 1; j >= 0; j--) {
  //         const parentLevel = levels[hierarchy[j].id];
  ...
  //     const node = _jm.add_node(parentId, id, content);
  ...
  var response = await get_children_suggestions(selected_node, _jm, document.getElementById("child-node-suggestion").value);
